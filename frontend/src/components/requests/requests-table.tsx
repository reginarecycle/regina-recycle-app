...
export default function RequestsTable({ onRefreshStats }: RequestsTableProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    "incoming" | "accepted" | "completed"
  >("incoming");
  const [selectedRequest, setSelectedRequest] = useState<RequestRow | null>(
    null,
  );
  const [completedOpen, setCompletedOpen] = useState(false); //changed
  ...
  const loadWalletBalance = useCallback(async () => {
    try {
      const result = await getCollectorWallet();
      const balance =
        result?.data?.balance ??
        result?.balance ??
  ...
  const handleCompleteRequest = async (updatedItems: CollectionItem[]) => {
    if (!selectedRequest) return;
    ...
    try {
      ...
      await completePickup(selectedRequest.pickupId, payload);
  ...
  const handleRejectRequest = async (reason: string, comments: string) => {
    ...
    //   pickupId: selectedRequest.pickupId,
    //   reason,
    //   comment: comments,
    // });
    try {
            ...
            render: (row:any) => (
              <span className="font-bold text-sm">
                ...
                {payoutLabel === "Payout ($)"
                  ...
                  : Number(row.estimatedEarning).toFixed(2)}
              </span>
            ),
          },
        ]
    ...
    <Card className="bg-white w-full gap-0 py-3">
      ...
      {selectedRequest && (
        <RequestDetailsModal
          ...
          sourceTab={activeTab}
          isCompatible={selectedRequest.Compatibility === 100}
        />
      )}
      ...
      <RejectRequestModal
        ...
        onConfirm={({ reason, comments }) => {
          // console.log("REJECT MODAL VALUES:", { reason, comments });
